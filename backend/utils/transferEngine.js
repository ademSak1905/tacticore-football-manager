...
async function negotiateTransfer(club, body = {}) {
  ...
  if (!player) return { status: 'error', message: 'Transfer listesindeki oyuncu bulunamad?.' };
  const category = categoryForPlayer(player, state.current_day);
  ...
  return {
    status: 'pending',
    message: `${player.name} i?in teklif g?nderildi. Cevap gelecek hafta Mesajlar b?l?m?ne d??ecek.`,
    responseWeek,
    responseDay
  };
}
  ...
  for (const offer of dueOffers) {
    ...
    if (status === 'club_accepted' && !wageOk) {
      ...
      const message = await createTransferInboxMessage(userId, {
        ...
        summary: `${offer.player_name} maa? teklifinizi yeterli bulmad?.`,
        body: `${offer.player_name}, ?nerilen ${money(offer.wage_offer)} maa?? yeterli bulmad?. Beklentisi yakla??k ${money(wageRequired)} seviyesinde.`,
        priority: 'important',
